...
def escape_markdown_v2(text: str) -> str:
    """
    Экранируем спецсимволы для Telegram MarkdownV2, включая точки в датах и URL.
    """
    # Список специальных символов для MarkdownV2
    special_chars = r'([_*\[\]()~`>#+\-=|{}.!])'
    # Экранируем специальные символы
    text = re.sub(special_chars, r'\\\1', text)
    # Дополнительно экранируем точки в датах (например, "17.05" → "17\.05")
...
def fetch_homeworks():
    ...
    try:
        ...
        logger.info(f"🗓 Получено {len(schedule)} уроков и {len(homeworks_data)} домашних заданий.")
        ...
        # Врубаем LLM, чтобы прокачать текстовую подачу
